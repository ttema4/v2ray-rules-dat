...
* В директории `domain/` хранятся текстовые списки доменов (например, `proxy-domain`).
* В директории `ip/` хранятся списки IP-адресов и CIDR-зон (например, `proxy-ip`).

## 🤝 Источники и благодарности

Этот проект опирается на работу opensource-сообщества. Отдельная благодарность авторам следующих проектов:

* 🗂 **[rekryt/iplist](https://github.com/rekryt/iplist)** — за актуальную и регулярно обновляемую базу доменов и IP-адресов, которая взята за основу этого репозитория.
* 🛠 **[v2fly/domain-list-community](https://github.com/v2fly/domain-list-community)** — за инструмент парсинга и сборки файла `geosite.dat`.
* 🛠 **[v2fly/geoip](https://github.com/v2fly/geoip)** — за компилятор для создания файла `geoip.dat`.
